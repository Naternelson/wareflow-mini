...
export const signinUser: RequestHandler = async (request: ApiRequest): Promise<AuthResponse> => {
	...
	if (typeof organizationId !== "number") user = await User.findOne({ where: { email }, include: ["organization"] });
	else user = await User.findOne({ where: { email, organizationId }, include: ["organization"] });
